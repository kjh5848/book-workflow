...
- **wrapper 이중 외곽 금지**: 그림은 `<figure>` 시맨틱 wrapper로 묶고 그 안에 그림 컴포넌트와 캡션을 형제로 둔다. `<figure>` wrapper는 외곽 없음 — 외곽 한 겹은 그림 컴포넌트(`.parser-flow` 컬러 외곽 또는 `.sp-figure` 회색 점선)가 가짐. 캡션은 항상 그림 박스 밖. 캡션 단일 진실원 → `structure.md`
