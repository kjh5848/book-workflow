...
### 톤
 - 서술어 통일: 기본 서술 문장은 무조건 **'하십시오체' (-습니다 / -입니다 / -합니다)** 로 엄격하게 통일
 - 구어체 금지: 개념이나 비유 설명 시에도 **해요체(-요, -죠, -거예요)** 절대 금지
 - 예외: 캐릭터 직접 대화(" ")와 내면 독백(*이탤릭*)에만 구어체 허용
 - 볼드체 띄어쓰기: 볼드(`**`) 내부에 괄호/따옴표 등 특수문자가 포함된 경우 닫힌 뒤 반드시 띄어쓰기 (`**통행증(Access Token)** 을`). 순수 텍스트는 붙여쓰기 (`**Docker**라는`).
 - 챕터 구조. 문제→기술소개→해결→결과
 - 닫기. "이것만은 기억하자" + 다음 예고
