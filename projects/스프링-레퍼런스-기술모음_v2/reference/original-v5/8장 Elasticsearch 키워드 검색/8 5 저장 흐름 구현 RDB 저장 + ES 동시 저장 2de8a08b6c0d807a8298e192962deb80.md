# 8.5 저장 흐름 구현: RDB 저장 + ES 동시 저장

---

## **1) 서비스 계층 설계**

---

**(확인) 경로: src/main/java/com/metacoding/spring_elasticsearch/ElasticSearch/ElasticSearchService.java**

### **1. RDB 저장 → ES 저장**

```java
@Transactional
public DeviceDocument saveDevices(DeviceDocument doc) {

    DeviceEntity entity = DeviceEntity.builder()
            .title(doc.getTitle())
            .content(doc.getContent())
            .build();

    DeviceEntity saved = deviceJpaRepository.save(entity);

    DeviceDocument savedDoc = new DeviceDocument(
            saved.getId(),
            saved.getTitle(),
            saved.getContent());

    deviceSearchRepository.save(savedDoc);

    return savedDoc;
}

```

RDB에 먼저 저장해 PK를 확보한 뒤, 동일한 ID로 ES 문서를 생성해 dual write(동시저장)를 수행합니다.

---

## **2) 트랜잭션 단위 처리 전략**

---

### **1. 단일 트랜잭션의 한계와 트레이드오프**

```java
@Transactional
public DeviceDocument saveDevices(DeviceDocument doc) { ... }
```

RDB와 ES는 서로 다른 저장소이기 때문에 하나의 트랜잭션으로 완전한 원자성을 보장할 수 없습니다. 현재 구조는 **RDB 저장 → ES 저장** 순서이며, ES 저장 실패 시 보상 처리(재시도/큐 적재 등)가 필요할 수 있습니다.

<aside>
💡

실습에서는 **RDB 저장 → ES 저장**을 한 번에 처리합니다. 다만 실무에서는 ES 저장 때문에 응답이 느려지거나 실패하는 상황을 줄이기 위해, **RDB 저장을 먼저 완료한 뒤 인덱싱 작업은 Kafka로 분리**하는 방법을 많이 사용합니다. 이때 서비스는 “ES에 저장해야 한다”는 이벤트를 Kafka로 발행하고, 별도 소비자가 이벤트를 받아 ES에 저장하며, 실패하면 재시도(리트라이)로 다시 처리합니다.

</aside>

---

## **3) 저장 API 구현 (POST)**

---

**(확인) 경로: src/main/java/com/metacoding/spring_elasticsearch/ElasticSearch/ElasticSearchSearchController.java**

```java
@PostMapping("/device")
public DeviceDocument saveDevice(@RequestBody DeviceDocument doc) {
    return elasticSearchService.saveDevices(doc);
}

```

POST 요청을 받으면 서비스의 `saveDevices()`가 호출되어 RDB와 ES가 동시에 저장됩니다.

---

### **4. 저장 흐름**

```mermaid
sequenceDiagram
    actor 클라이언트
    participant 컨트롤러
    participant 서비스
    participant RDB
    participant ES

    클라이언트->>컨트롤러: POST /device (title, content)
    컨트롤러->>서비스: saveDevices(doc)
    서비스->>RDB: save(entity)
    RDB-->>서비스: saved(id)
    서비스->>ES: save(document)
    ES-->>서비스: savedDoc
    서비스-->>컨트롤러: savedDoc
    컨트롤러-->>클라이언트: 200 OK + savedDoc

```

요청이 들어오면 컨트롤러가 서비스에 전달하고, 서비스는 RDB 저장 후 ES에 문서를 저장한 뒤 최종 결과를 반환합니다.

---