...
`.ts`는 **영상 조각 파일**이라 일반 바이너리 타입(`MediaType.APPLICATION_OCTET_STREAM`)으로 응답합니다.
...
`quality` 값으로 **폴더를 선택**합니다. 예: 720p, 1080p
...
크롬/엣지는 **Hls.js**로 재생합니다. 사파리는 HLS를 기본 지원하므로 `video.src`로 바로 재생합니다.
