...
실습은 동기 방식으로 “RDB 저장 + ES 저장”을 같이 수행하지만, 실무에서는 RDB 저장 후 “인덱싱 이벤트”를 발행하고 Elasticsearch 저장은 별도 작업자가 처리하는 구조를 자주 사용합니다. 예를 들어 Kafka 같은 메시지 큐로 “문서 업데이트 이벤트”를 보내고, Consumer가 ES에 저장합니다. 이렇게 하면 저장 API 응답은 빨라지고, ES 장애가 있어도 이벤트를 재처리(리트라이)하여 결국 검색 데이터가 따라오게 만들 수 있습니다.
