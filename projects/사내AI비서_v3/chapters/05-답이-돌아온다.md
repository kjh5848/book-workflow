...
<div class="scene-actors">
  <div class="scene-grid">
    <div class="scene-actor">
      ...
      <div class="scene-note">조선 과거 제도가<br>궁금한 사람</div>
    </div>
    <div class="scene-bridge">
      <div class="scene-line"><span class="scene-line-label">질문</span><span class="scene-ar">→</span></div>
      <div class="scene-line reverse"><span class="scene-ar">←</span><span class="scene-line-label">정리된 답변</span></div>
    </div>
    <div class="scene-actor center">
      <div class="scene-icon lib"></div>
      <div class="scene-role">사서<em>RAG Q&amp;A</em></div>
      <div class="scene-note">책을 꺼내 읽고<br>출처와 함께 답하기</div>
    </div>
    <div class="scene-bridge">
...
*그림 5-2. 사서가 방문자와 서가 사이에 서서 답을 만들어 줍니다. 가운데 사서가 이번 챕터의 LCEL 파이프라인입니다*

검색만 했던 챕터 4까지는 사서가 책을 꺼내 던져 주는 데서 멈췄습니다. 이번에는 사서가 그 책을 펼쳐 읽고, 출처를 적어 답변을 건넵니다. 단계로 보면 1~2번이 챕터 4까지였고, 3~4번이 이번 챕터에서 더해집니다.
...
<div class="librarian-flow">
  ...
  <div class="lf-caption">그림 5-3. RAG Q&amp;A 흐름. 챕터 4까지가 1~2번이었고, 이번 챕터에서 3~4번을 추가합니다</div>
</div>
...
<div class="lcel-pipeline">
  ...
  <div class="lp-caption">그림 5-4. LCEL 파이프라인 구조. 앞 블록의 출력이 <code>|</code>를 타고 다음 블록의 입력이 됩니다</div>
</div>
...
파이프 연산자(`|`)가 각 단계의 출력을 다음 단계의 입력으로 넘깁니다. 위에서 본 그림 5-4 다이어그램이 그대로 코드 한 덩어리가 됐습니다. `itemgetter("question")` 은 입력 dict에서 `question` 키만 꺼내는 호출가능 객체로, 검색 단계로 흘러가는 입력을 골라 줍니다.
...
*그림 5-5. 6번째 메모가 들어오면 가장 오래된 1번째가 빠집니다. 항상 `maxlen = 5`*
...
*그림 5-8. 멀티턴 대화. 이전 질문을 기억해 "거기", "그럼"을 해석합니다*
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 5-9. 이번 챕터의 LCEL 파이프라인은 다음 챕터에서 에이전트 도구가 됩니다</div>
