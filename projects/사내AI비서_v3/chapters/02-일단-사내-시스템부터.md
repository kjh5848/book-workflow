...
이번 챕터는 **실행만** 해보겠습니다. 코드는 읽지 않아도 됩니다.

![](../assets/CH02/diagram/02_exercise-flow.png)
*그림 2-1. 이번 챕터의 실습 흐름. 서버 시작에서 관리자 웹 UI 확인까지 세 단계입니다*
...
![](../assets/CH02/image/02_chapter-opening.png)
*그림 2-2. 문서 검색만으로는 답할 수 없는 질문이 있습니다*
...
팀장이 화이트보드 쪽으로 자리를 옮기더니 한마디 더 던졌습니다.

**팀장**: "AI 입장에선 직원 정보가 식당 주방 안 재고 같은 거예요. 직접 들어가서 꺼내면 안 되죠. 종업원한테 시켜야죠."

종업원. 손님이 "치킨 샐러드 하나"라고 말하면 종업원이 주방에 전달하고, 음식이 나오면 다시 손님 자리로 가져옵니다. 손님은 주방을 보지 않습니다. AI도 마찬가지입니다. PostgreSQL을 직접 건드리는 게 아니라 **누군가에게 부탁하는 창구**가 필요했습니다.

![](../assets/CH02/diagram/02_api-waiter.png)
*그림 2-3. 손님(AI)은 주방(DB)에 직접 가지 않습니다. 종업원(API)이 주문을 받아 결과를 가져옵니다*

이 종업원이 곧 **REST API**입니다. 정식 정의는 챕터 끝 용어 정리 표에서 마저 합니다.

그래서 이번 챕터는 AI 비서보다 **사내 시스템**을 먼저 실행해 보겠습니다. 코드를 한 줄씩 뜯지는 않습니다. 완성된 시스템을 띄워 "이런 데이터가 이런 모양으로 들어온다"를 눈으로 확인하는 것이 목적입니다.
...
![](../assets/CH02/image/02_erd-diagram.png)
*그림 2-4. 세 테이블의 관계. 직원을 중심으로 연차가 연결되고, 매출은 부서 단위로 독립 관리됩니다*

세 테이블 모두에 **CRUD**(등록, 조회, 수정, 삭제) 네 동작을 제공해야 합니다. 메뉴판으로 치면 한 식당이 갖춰야 할 네 종류 주문(만들기·읽기·수정·지우기)인 셈입니다.

![](../assets/CH02/image/02_crud-menu.png)
*그림 2-5. 데이터 한 종류당 갖춰야 할 네 동작. 등록·조회·수정·삭제가 한 세트입니다*

세 테이블에 각각 이 네 동작을 모두 제공하는 것이 이번 챕터에서 띄울 시스템입니다. 챕터 6에서 커넥트HR 에이전트가 이 API를 직접 호출하게 되니, 지금은 **"AI가 나중에 쓸 시스템을 먼저 열어두는"** 단계입니다.
...
*그림 2-6. FastAPI가 자동 생성한 Swagger UI. 직원, 연차, 매출 세 영역의 API가 한눈에 보입니다*
...
직원 목록을 조회하는 `GET /api/employees`를 펼치고 **Try it out** 버튼을 누른 뒤 **Execute** 를 누르면, 시드 데이터로 들어 있는 직원 다섯 명이 JSON 배열로 돌아옵니다.

![](../assets/CH02/terminal/02_api-test-employee.png)
*그림 2-7. Swagger UI에서 직원 목록 API 호출 결과. 시드 데이터 5명이 사번 순으로 돌아옵니다*
...
*그림 2-8. Jinja2 템플릿으로 만든 관리자 대시보드. 직원, 연차, 매출 현황을 한눈에 보여 줍니다*

좌측 **직원 관리** 메뉴로 이동하면 등록 폼과 현재 직원 목록이 한 화면에 나옵니다. 이름 "홍길동"과 부서, 직급, 입사일을 입력하고 저장을 누릅니다.

![](../assets/CH02/terminal/02_admin-employee-create.png)
*그림 2-9. 직원 등록 폼. 이름·부서·직급·입사일만 채우면 사번은 시스템이 부여합니다*

저장과 동시에 아래 목록에 한 줄이 바로 추가됩니다. 사번은 시스템이 자동으로 부여합니다.

![](../assets/CH02/terminal/02_admin-employees-list.png)
*그림 2-10. 직원 관리 목록. 맨 아래에 홍길동이 새 사번으로 추가된 모습입니다. API 호출법을 몰라도 CRUD가 됩니다*
...
<div class="arch-fullmap">
  <div class="arch-fullmap-title">전체 구성도. 짙은 박스가 챕터 2 범위</div>
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row afm-three">
      <div class="afm-box afm-faint">
        <div class="afm-label">Tool Calling Agent</div>
        <div class="afm-sub">ReAct 패턴 · 사후 분류</div>
      </div>
      <div class="afm-box afm-faint">
        <div class="afm-label">MCP Tools</div>
        <div class="afm-sub">DB 조회 · 문서 검색</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row">
      <div class="afm-box afm-faint">
        ...
        <div class="afm-sub">검색기 → 프롬프트 → LLM</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row afm-three">
      ...
      <div class="afm-box afm-faint afm-dashed">
        <div class="afm-label">사내 문서 (PDF·DOCX)</div>
        <div class="afm-sub">취업규칙·복지·매출보고</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row">
      <div class="afm-box afm-on">
        <div class="afm-tag">오늘 띄운 DB</div>
        <div class="afm-label">PostgreSQL</div>
        <div class="afm-sub">직원 · 연차 · 매출</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-note">
    오늘 띄운 건 <b>FastAPI 대시보드</b>와 <b>PostgreSQL</b>입니다. 나머지는 챕터 3부터 하나씩 채워집니다.
  </div>
</div>

*그림 2-11. 챕터 2가 만든 부분. 짙은 박스 두 개가 오늘 띄운 시스템입니다*
...
세 테이블이 어떻게 묶이는지 한눈에 보겠습니다. 같은 자원에 대해 동작이 다른 메서드(GET·POST·PATCH·DELETE)가 한 그룹으로 묶입니다.

<div class="sp-figure">
  <div class="sp-figure-title">그림 2-12. API 엔드포인트 세 그룹. 자원별로 묶고 동작별로 색을 분리했습니다</div>
  <div class="sp-flow">
    <div class="sp-step">
      <span class="sp-step-num">직원</span>
      <div class="sp-step-title">/api/employees</div>
      <div class="sp-step-desc">5개 (CRUD + 상세)</div>
    </div>
    <div class="sp-flow-arrow">·</div>
    <div class="sp-step">
      <span class="sp-step-num">연차</span>
      <div class="sp-step-title">/api/leaves</div>
      <div class="sp-step-desc">4개 (조회·등록·사용·수정)</div>
    </div>
    <div class="sp-flow-arrow">·</div>
    <div class="sp-step">
      <span class="sp-step-num">매출</span>
      <div class="sp-step-title">/api/sales</div>
      <div class="sp-step-desc">3개 (조회·등록·합계)</div>
    </div>
  </div>
</div>
