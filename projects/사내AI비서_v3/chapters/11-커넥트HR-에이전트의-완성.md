# 챕터 11. 커넥트HR 에이전트의 완성. 레이어드 아키텍처로 조립

:::goal
**이번 챕터가 끝나면**

- 챕터 4부터 10까지 따로 자란 부품을 **레이어드 아키텍처**로 조립합니다
- API 게이트웨이부터 저장소까지 한 줄로 꿰어진 `RagPipeline`을 만들어 봅니다
- 답변에 **근거**를 붙입니다. 정형 질문은 JSON, 비정형 질문은 PDF 페이지 이미지
- 챕터 7의 채팅 UI를 재사용해 실제 사용 화면에서 마지막 점검을 합니다
- **11챕터에 걸친 여정**을 한 장의 시스템 아키텍처에 담아 봅니다
:::
...
1. `src/pipeline.py`. 챕터 8~10에서 고른 D 조합(Semantic 청킹·하이브리드 검색·리랭커·약어 확장·Parent Doc·Vision 파서)을 한 파일에 조립합니다
2. `src/evidence_capture.py`. PDF 페이지를 PNG로 렌더링해 근거 이미지로 만듭니다
3. `app/chat_api.py` + `templates/chat.html`. 정형(JSON)과 비정형(이미지) 근거 분기를 붙입니다
::::

## 11.1 부품을 한 줄로 - 레이어드 아키텍처

팀장이 의자를 옆자리로 밀고 앉았습니다. 모니터엔 챕터 8~10에서 돌린 평가표가 그대로 떠 있었습니다. D 조합의 환각률 0.871이 형광펜으로 칠해진 것처럼 또렷했습니다.

**팀장**: "이번엔 새 이론 안 얹어요. 지금까지 본 부품을 한 자리에 가지런히 모으기만 하면 됩니다."
...
**동료**: "그냥 함수 줄줄이 import해서 한 파일에 다 부르면 안 되나요?"
...
**팀장**: "위는 지휘하는 층, 가운데는 일하는 부품, 아래는 외부 저장소예요. 위에서 아래로만 부르고 옆은 안 봐요."
...
<div class="sp-figure">
<div class="sp-figure-title">그림 11-1. 레이어드 아키텍처. 위에서 아래로만 호출이 흐릅니다</div>

<div class="sp-flow">
<div class="sp-step accent">
<span class="sp-step-num">APPLICATION</span>
<div class="sp-step-title">RagPipeline</div>
<div class="sp-step-desc">네 부품을 순서대로 호출하는 지휘자<br><code>src/pipeline.py</code></div>
</div>
<div class="sp-flow-arrow">↓</div>
<div class="sp-step info">
<span class="sp-step-num">DOMAIN</span>
<div class="sp-step-title">튜닝 부품 4종</div>
<div class="sp-step-desc">QueryExpander · EnsembleRetriever · CrossEncoderReranker · ParentDocumentRetriever<br><code>src/tuning/</code></div>
</div>
<div class="sp-flow-arrow">↓</div>
<div class="sp-step">
<span class="sp-step-num">INFRASTRUCTURE</span>
<div class="sp-step-title">외부 저장소</div>
<div class="sp-step-desc">ChromaDB · rank-bm25 · CE 모델 · parent_docs.json<br><code>src/tools/</code></div>
</div>
</div>

</div>

세 층이 한 방향으로만 얽히기 때문에, 한 층의 내부를 바꿔도 다른 층은 영향이 적습니다. 가령 벡터DB를 Qdrant로 갈아끼울 땐 Infrastructure만 손대면 됩니다. 약어 사전을 사내 용어집으로 교체할 땐 Domain의 `QueryExpander`만 고치면 됩니다. 위층은 아무것도 모릅니다.

가운데 칸 Domain에 놓인 네 부품이 한 질문 안에서 어떻게 손을 잡는지를 한 줄에 펴면 이렇게 됩니다.

<div class="pipeline-box">
<div class="pipeline-title">그림 11-2. RAG 엔진. 질문이 들어와 답변이 나가기까지</div>
<div class="pipeline">
<div class="step-card">
<div class="sc-num">01</div>
<div class="sc-title">QueryExpander</div>
<div class="sc-desc">사내 약어를 풀어 질문을 보강</div>
<div class="sc-meta">expand()</div>
</div>
<div class="flow-arrow">→</div>
<div class="step-card">
<div class="sc-num">02</div>
<div class="sc-title">EnsembleRetriever</div>
<div class="sc-desc">BM25와 ChromaDB로 자식 청크 검색</div>
<div class="sc-meta">search()</div>
</div>
<div class="flow-arrow">→</div>
<div class="step-card">
<div class="sc-num">03</div>
<div class="sc-title">CrossEncoderReranker</div>
<div class="sc-desc">딥러닝 모델로 정합성 채점</div>
<div class="sc-meta">rerank()</div>
</div>
<div class="flow-arrow">→</div>
<div class="step-card">
<div class="sc-num">04</div>
<div class="sc-title">ParentDocumentRetriever</div>
<div class="sc-desc">자식 청크에서 부모 페이지 복원</div>
<div class="sc-meta">resolve()</div>
</div>
</div>
</div>

`RagPipeline`이 질의가 들어올 때마다 이 네 부품을 차례로 부르고, 그 부품들은 아래층 `src/tools/`의 ChromaDB·BM25·`parent_docs.json`을 끌어 씁니다. 벡터DB는 `ex11/data/docs`를 자체 인덱싱해 자식·부모 두 저장소로 빌드합니다. 페이지 단위 원문은 부모로 `ex11/data/parent_docs.json`에 두고, 220자로 잘게 쪼갠 자식 청크는 ChromaDB에 `parent_id` 메타데이터와 함께 적재합니다. PDF는 `src/tuning/hybrid_parser.py`(챕터 10의 하이브리드 파서를 이식한 모듈)가 pypdf로 먼저 꺼내 보고 스캔본이면 Vision LLM으로 넘어가 텍스트를 뽑습니다.
...
`__init__`에서 도메인 객체 네 개를 한 번 준비해 두고, `run()`에서는 다섯 줄 안에 **확장 → 자식 검색 → 리랭크 → 부모 복원 → 절단**을 순서대로 부릅니다. 객체별 내부 구현은 `src/tuning/` 각 파일에 흩어져 있어서 부품 하나만 따로 교체하거나 테스트할 수 있습니다.
...
## 11.2 근거의 모양은 질문이 정한다

조립이 끝나도 한 가지가 남았습니다. **근거**입니다. 답이 정확해도 어디서 가져왔는지 보이지 않으면 사용자는 끝까지 의심합니다. 검색 결과에는 `(source, page)` 쌍이 이미 붙어 있으니, 그 정보로 PDF의 해당 페이지를 PNG로 떠 주면 답변 옆에 바로 붙일 수 있습니다.

`src/evidence_capture.py`를 열고 TODO의 `pass`를 지우고 아래 코드를 작성합니다.

```python [실습 2] src/evidence_capture.py. PDF 페이지 → PNG 렌더 + 캐시
import fitz  # PyMuPDF
...
`source_name`은 `test_questions.json`의 `relevant_sources`와 같은 PDF 파일명(확장자 제외)입니다. `page_num`은 1부터 시작하는 페이지 번호입니다. `dpi`가 클수록 선명하지만 파일도 커집니다. 화면 표시용으로는 150이면 충분합니다.
...
**동료**: "서약서나 취업규칙은 이렇게 이미지로 보는 게 좋네요. 그런데 '1분기 매출 얼마야' 같은 질문은 이미지보다 숫자 그 자체가 더 편해요. 표는 표, JSON이면 JSON으로 보여 주는 게 낫겠어요."
...
```python [실습 3] app/chat_api.py. 카테고리 분류 + 근거 모양 분기
import re
...
분류 결과는 `ChatResponse.category` 필드로 같이 내려보냅니다. 그리고 기존 엔드포인트 로직에서 두 갈래로 갈립니다. `category == "structured"` 이면 DB 조회 결과를 `evidence_json` 필드에 담고, `category == "unstructured"` 이고 아직 `evidence_images`가 비어 있으면 상위 문서의 (source, page)로 `render_evidence_image()`를 불러 이미지를 첨부합니다. 두 갈래가 한 엔드포인트 안에서 자연스럽게 갈라집니다.

<div class="sp-compare">
<div class="sp-compare-block accent">
<div class="sp-compare-label">정형 질문 → JSON</div>
<div class="sp-compare-content">
<b>Q.</b> 영업부 11월 매출 얼마야?<br><br>
영업부 11월 매출은 <b>1,240,500,000원</b>입니다.<br><br>
<code>{"team":"영업부","month":"2025-11","total":1240500000}</code>
</div>
</div>
<div class="sp-compare-block warm">
<div class="sp-compare-label">비정형 질문 → 이미지</div>
<div class="sp-compare-content">
<b>Q.</b> 병가 신청 절차 알려줘<br><br>
병가는 진단서 제출 후 인사팀에 신청합니다...<br><br>
<i>[PDF 캡처] HR_취업규칙 p.3</i>
</div>
</div>
</div>
...
<div class="tl-body">
<div class="tl-dim">─── ex11 커넥트HR 에이전트 ───</div>
<div class="tl-dim">VISION_MODEL=<span class="tl-str">qwen2.5vl:7b</span>  ·  PORT=<span class="tl-num">8000</span></div>
<div class="tl-section"><span class="tl-label">1. 벡터DB 준비</span></div>
<div class="tl-kv-row"><span class="tl-val">✓</span> 임베딩 모델 준비 완료</div>
<div class="tl-kv-row"><span class="tl-val">✓</span> 기존 인덱싱 재사용 — children=<span class="tl-num">39</span>, parents=<span class="tl-num">7</span></div>
<div class="tl-section"><span class="tl-label">2. Cross-Encoder 리랭커 로드</span></div>
<div class="tl-kv-row"><span class="tl-val">✓</span> 리랭커 준비 완료</div>
<div class="tl-section"><span class="tl-label">3. 서버 기동</span></div>
<div class="tl-kv-row"><span class="tl-val">✓</span> 준비 완료 → <span class="tl-num">http://localhost:8000/chat</span></div>
...
브라우저에서 `http://localhost:8000/chat`을 엽니다. 챕터 7의 ConnectHRAgent 채팅 UI가 그대로 떴습니다. 같은 UI, 다른 엔진.
...
*그림 11-5. 병가 규정이 정확히 돌아옵니다. 출장·급여 같은 다른 주제는 섞이지 않습니다*
...
<div class="tl-chrome">
<div class="tl-traffic"><span></span><span></span><span></span></div>
<div class="tl-title">ex11 — POST /api/chat</div>
<div class="tl-spacer"></div>
</div>
<div class="tl-body">
...
사전에 등록된 `병가`가 `병가(병가 유급휴가)`로 풀어져 검색으로 넘어갔습니다. 이어서 llama3.1 모델이 검색 결과로 답을 만드는 데 24.9초가 걸렸고, 유형은 비정형(`unstructured`)으로 기록됩니다. 챕터 7에서 붙인 TokenTracker가 챕터 11에서도 그대로 일하고 있습니다.

다음은 약어가 섞인 질문. `WFH 정책 알려줘`.

![](../assets/CH11/terminal/11_chat-wfh.png)
*그림 11-7. 취업규칙의 재택·원격 근무 조항이 근거로 돌아옵니다*
...
사전이 `WFH`에 `(재택근무)`를 덧붙였습니다. 그런데 이 확장어는 문서에 없는 표현입니다. 그 간극을 벡터 임베딩이 메워 줍니다. `재택근무`와 `원격 근무`는 의미 공간에서 가까운 자리에 놓여 있어 한쪽을 질문으로 들이밀면 다른 쪽 문서가 걸립니다. 챕터 9의 약어 사전이 못 덮는 표현 차이를 챕터 4의 임베딩 모델이 메우는 협업입니다. 출력 토큰이 226에서 238로 조금 늘어난 것은 LLM이 워케이션·거점 오피스 두 항목을 합쳐 답했기 때문입니다.

마지막 질문. `정보보안서약서 내용이 뭐야?`.

![](../assets/CH11/terminal/11_chat-security.png)
*그림 11-9. 제4조 생성형 AI 활용 지침까지 조항 번호가 또렷하게 돌아옵니다*
...
<div class="tl-section"><span class="tl-label">QueryExpander</span> <span class="tl-dim">약어 매칭 없음 · 원본 그대로 검색</span></div>
<div class="tl-section"><span class="tl-label">TokenTracker 기록</span></div>
<div class="tl-kv">모델 <span class="tl-str">llama3.1:8b</span></div>
<div class="tl-kv">입력 토큰 <span class="tl-num">6</span></div>
<div class="tl-kv">출력 토큰 <span class="tl-num">226</span></div>
...
**동료**: "질문 타입이 세 개 다 다른데 세 번 다 근거까지 붙어요."

## 11.4 엔진의 내부 - 6단계로 한 줄에
...
11.4의 파이프라인이 "질문이 답변으로 흐르는 길"이었다면, 아래 구성도는 그 길을 떠받치는 **시스템 전체**를 한 장에 담은 모습입니다. 사용자부터 LLM, 에이전트 루프, 도구, 저장소까지.
...
<div class="sp-figure">
<div class="sp-figure-title">그림 11-13. 11챕터의 여정. 환각에서 출발해 시스템으로 끝났습니다</div>

<div class="sp-row warm">
<span class="sp-row-label">CH 1</span>
<span class="sp-row-value">환각과 RAG의 첫 만남. LLM이 회사 문서를 본 적 없다는 사실을 직접 확인했습니다</span>
</div>
<div class="sp-row info">
<span class="sp-row-label">CH 2~3</span>
<span class="sp-row-value">사내 시스템과 문서 설계. FastAPI로 직원·연차·매출 DB를 만들고 어떤 문서를 어떻게 정리할지 정했습니다</span>
</div>
<div class="sp-row info">
<span class="sp-row-label">CH 4~5</span>
<span class="sp-row-value">벡터 DB와 RAG 엔진. PDF·DOCX를 파싱해 ChromaDB에 적재하고 출처가 함께 오는 RAG 엔진을 조립했습니다</span>
</div>
<div class="sp-row info">
<span class="sp-row-label">CH 6~7</span>
<span class="sp-row-value">통합 에이전트와 운영. ReAct 루프로 도구 호출을 처리하고 캐시·토큰 추적을 얹어 운영 안정화를 했습니다</span>
</div>
<div class="sp-row info">
<span class="sp-row-label">CH 8~10</span>
<span class="sp-row-value">검색 품질·질문 이해·멀티모달. 하이브리드 검색·리랭킹·약어 확장·Parent Doc·Vision 파서로 답변 품질을 끌어올렸습니다</span>
</div>
<div class="sp-row accent">
<span class="sp-row-label">CH 11</span>
<span class="sp-row-value">레이어드 아키텍처로 조립. 모든 부품이 한 시스템 안에서 자기 자리를 찾았고, 답변에 근거가 붙었습니다</span>
</div>
...
**팀장**: "이걸로 다음 주 시연 가요. 직원들이 직접 써 봐야 진짜 평가가 나오니까요."
...
- **11챕터의 자리가 한 장에 모였습니다.** 챕터 1의 환각 체험에서 출발해, 챕터 2~3의 사내 시스템·문서 설계, 챕터 4~5의 벡터DB와 RAG 엔진, 챕터 6~7의 통합 에이전트와 운영, 챕터 8~10의 검색 품질·질문 이해·멀티모달까지 챕터마다 한 박스씩 짙어졌고 챕터 11에서 그 모든 박스가 시스템 아키텍처 한 장에 박혔습니다.
- **레이어드 아키텍처는 부품을 가지런히 놓는 일입니다.** Application은 지휘하고, Domain은 부품을 두고, Infrastructure는 외부 저장소만 다룹니다. 위에서 아래로만 부르기 때문에 한 층의 내부를 바꿔도 다른 층은 흔들리지 않습니다.
- **근거의 모양은 질문이 정합니다.** 정형 수치 질문은 JSON으로, 비정형 문서 질문은 PDF 캡처 이미지로. 사용자는 답의 내용뿐 아니라 어디서 나왔는지로 신뢰를 판단합니다.
- **시스템은 책의 끝이 아니라 운영의 시작입니다.** 평가셋을 늘리고, 실패한 질문을 모으고, Langfuse 같은 도구로 운영 중 품질을 상시 관측하세요. 커넥트HR 에이전트는 이제 사용자 손에서 자랍니다.
:::
