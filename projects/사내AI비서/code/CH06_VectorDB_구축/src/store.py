...
def get_or_create_collection(
    ...
    if doc_count > 0:
        print(f"  기존 컬렉션 로드: '{collection_name}' (기존 문서 수: {doc_count})")
    else:
...
def embed_chunks(
    ...
    print(f"  임베딩 완료: {len(all_embeddings)}개 벡터 (차원: 768)")
...
def store_chunks_to_chroma(
    ...
    print(f"\nChromaDB 초기화: {chroma_dir_display}")
    try:
        client = chromadb.PersistentClient(
            path=chroma_dir,
    ...
    try:
        for batch_start in range(0, len(ids), BATCH_SIZE):
            ...
            done = min(batch_end, len(ids))
            print(f"  업서트 완료: {done}/{len(ids)}개", end="\r", flush=True)
    except Exception as e:
        raise RuntimeError(
    ...
    print(f"\nChromaDB 저장 완료! (컬렉션 총 문서 수: {final_count})")
