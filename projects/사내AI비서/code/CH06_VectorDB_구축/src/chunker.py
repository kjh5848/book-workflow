...
def build_image_caption_chunk(
    ...
    """이미지 캡션을 텍스트 청크로 변환한 딕셔너리를 생성합니다.

    이미지/표/차트 설명을 일반 텍스트 청크와
    동일한 형식으로 변환하되, 이미지 경로를 메타데이터에 포함합니다.
    이를 통해 검색 결과에서 관련 이미지 캡처본을 참조할 수 있습니다.

    Args:
        caption: 이미지/표/차트 LLM 설명 텍스트
        image_path: 캡처본(PNG) 파일 경로
        doc_id: 문서 고유 ID
        file_name: 원본 파일명
        source_path: 원본 파일 절대 경로
        page: 출처 페이지 번호
    ...
    Returns:
        {
            "id": 청크 고유 ID (str),
            "text": "[이미지 캡션] " + 캡션 텍스트 (str),
            "metadata": {메타데이터 + image_path 포함},
...
def chunk_all_documents(
    ...
    for result in extract_results:
        ...
        image_count = sum(1 for c in chunks if c["chunk_type"] == "image_caption")
        print(
            f"  청킹 완료: {file_name} → "
            f"텍스트 청크 {text_count}개, 이미지 캡션 청크 {image_count}개"
        )
