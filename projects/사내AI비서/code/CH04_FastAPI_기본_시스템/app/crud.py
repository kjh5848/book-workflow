...
def create_employee(
    ...
    Raises:
        psycopg2.errors.UniqueViolation: 동일 사번이 이미 존재할 때
    """
    # [PROCESS] INSERT 후 RETURNING으로 생성된 행 반환
    sql = """
...
def get_employee_by_id(
    ...
    Returns:
        Employee 객체 또는 None
    """
    sql = "SELECT id, emp_no, name, dept, position, hire_date FROM employee WHERE id = %s"
    with conn.cursor() as cur:
...
def update_employee(
    ...
    Returns:
        수정된 Employee 객체 또는 None (해당 ID 없음)
    """
    ...
    if not set_parts:
        return get_employee_by_id(conn, employee_id)
...
def get_all_leaves(
    ...
    """연차 잔여량 전체 목록을 조회합니다 (직원 이름 조인 포함).
    ...
    Returns:
        LeaveBalance 객체 리스트
    """
    # [PROCESS] employee 테이블과 JOIN하여 이름 포함 조회
    base_sql = """
...
def update_leave_usage(
    ...
    """
    # [PROCESS] used_days 누적 + remaining_days 재계산
    check_sql = """
    ...
    """
    with conn.cursor() as cur:
        cur.execute(check_sql, (employee_id, year))     # ① 잔여 확인
        ...
        cur.execute(update_sql, (days, employee_id, year))  # ③ 사용 일수 누적
        row = cur.fetchone()
