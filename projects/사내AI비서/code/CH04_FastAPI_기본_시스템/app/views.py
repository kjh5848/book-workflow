    ...
    emp_dicts = [
        {
            ...
            "name": e.name,
            "dept": e.dept,
            "email": e.emp_no,          # 화면에서 사번을 이메일 위치에 표시
...
def create_employee_view(
    ...
    try:
        with get_connection() as conn:
            crud.create_employee(conn, emp_no, name, dept, position, parsed_date)  # ②
...
def update_employee_view(
    ...
    try:
        with get_connection() as conn:
            crud.update_employee(conn, employee_id, name_val, dept_val, pos_val, parsed_date)
    except Exception as exc:
...
def record_leave_usage(
    ...
    try:
        with get_connection() as conn:
            crud.update_leave_usage(conn, employee_id, days)       # ①
...
def update_leave_view(
    ...
    Args:
        ...
        employee_id: 직원 ID (폼 구조 일치용, 미사용)
        year:        연도 (폼 구조 일치용, 미사용)
        total:       수정할 총 연차
        used:        수정할 사용 연차
        remaining:   잔여 (DB 자동 계산, 폼 구조 일치용)
    ...
    try:
        with get_connection() as conn:
            crud.update_leave_balance(conn, leave_id, total, used)  # ①
    except Exception as exc:
