...
@router.get("/employees/{employee_id}", response_model=EmployeeResponse, summary="직원 단건 조회")
def api_get_employee(employee_id: int) -> EmployeeResponse:
    """직원 ID로 단일 직원 정보를 반환합니다.

    Args:
        employee_id: 직원 PK (URL 경로)

    Returns:
        EmployeeResponse

    Raises:
        HTTPException 404: 해당 ID의 직원이 없을 때
    """
    try:
        with get_connection() as conn:
            emp = crud.get_employee_by_id(conn, employee_id)       # ①
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail="데이터베이스에 연결할 수 없습니다.") from exc
    ...
    if emp is None:
        raise HTTPException(                                        # ②
            status_code=404,
            detail=f"직원 ID {employee_id}를 찾을 수 없습니다."
        )
...
def api_create_employee(body: EmployeeCreate) -> EmployeeResponse:
    ...
    Raises:
        HTTPException 409: 동일 사번이 이미 존재할 때
    """
    try:
        with get_connection() as conn:
...
@router.patch("/employees/{employee_id}", response_model=EmployeeResponse, summary="직원 수정")
def api_update_employee(employee_id: int, body: EmployeeUpdate) -> EmployeeResponse:
    ...
    if emp is None:
        raise HTTPException(status_code=404, detail=f"직원 ID {employee_id}를 찾을 수 없습니다.")
...
@router.delete("/employees/{employee_id}", status_code=204, summary="직원 삭제")
def api_delete_employee(employee_id: int) -> None:
    """직원을 삭제합니다.
    ...
    try:
        with get_connection() as conn:
            deleted = crud.delete_employee(conn, employee_id)      # ①
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail="데이터베이스에 연결할 수 없습니다.") from exc
...
def api_use_leave(body: LeaveUsageRequest) -> LeaveBalanceResponse:
    ...
    Args:
        body: LeaveUsageRequest (JSON body) — employee_id + days
    ...
    try:
        with get_connection() as conn:
            lb = crud.update_leave_usage(conn, body.employee_id, body.days)  # ①
    ...
    if lb is None:
        raise HTTPException(
            status_code=404,
            detail=f"직원 ID {body.employee_id}의 연차 레코드를 찾을 수 없습니다."
        )
...
@router.patch("/leaves/{leave_id}", response_model=LeaveBalanceResponse, summary="연차 수정")
def api_update_leave(leave_id: int, body: LeaveBalanceUpdate) -> LeaveBalanceResponse:
    """연차 잔여량을 직접 수정합니다.

    Args:
        leave_id: leave_balance PK (URL 경로)
    ...
    Raises:
        HTTPException 404: 해당 ID의 레코드가 없을 때
    """
    try:
        with get_connection() as conn:
            lb = crud.update_leave_balance(conn, leave_id, body.total_days, body.used_days)  # ①
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail="데이터베이스에 연결할 수 없습니다.") from exc

    if lb is None:
        raise HTTPException(status_code=404, detail=f"연차 ID {leave_id}를 찾을 수 없습니다.")
