...
튜닝을 한 가지씩 더할 때마다 코드가 여기저기 흩어졌습니다. 청킹 재설계, 하이브리드 검색, 리랭킹, 쿼리 풀어쓰기, 비전 OCR이 각자 다른 챕터에 한 줌씩 있었습니다. 한 번 더 정리가 필요했습니다. 질문이 들어오는 입구는 하나로, 답이 나가는 출구도 하나로. 뒷단에서 사서가 무엇을 부르든 바깥에서는 같은 모양이 보이게. 부품별로 책임을 쪼개고 한 파이프라인에 엮었습니다.
