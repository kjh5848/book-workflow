...
<!-- [GEMINI PROMPT: 01_llm-hallucination]
path: assets/CH01/01_llm-hallucination.png
...
Left side: minimalist line-art person icon labeled '나 (개발자)' with a speech bubble
containing '커넥트 연차 규정 알려줘'.
Center: a thin arrow pointing right toward a minimalist line-art brain icon labeled 'LLM'.
The brain has a thought bubble showing a bookshelf labeled '수십억 권의 학습 데이터'
with a small X mark over a document labeled '커넥트 사내규정'.
Right side: a speech bubble from the brain containing '입사 1년 미만 시 매월 1일...'
with a small warning triangle icon and Korean text '환각 (Hallucination)' below.
Style: architecture-infographic
-->
![LLM 환각 개념도](../assets/CH01/01_llm-hallucination.png)
...
<!-- [CAPTURE NEEDED: 01_hallucination-result
  path: assets/CH01/01_hallucination-result.png
  desc: `python main.py` 실행 후 LLM이 커넥트 연차 규정에 대해 환각 답변을 출력한 터미널 화면
] -->
![환각 답변 실행 결과](../assets/CH01/01_hallucination-result.png)
