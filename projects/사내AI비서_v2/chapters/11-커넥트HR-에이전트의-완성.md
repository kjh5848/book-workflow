...
벡터DB는 `ex11/data/docs`를 자체 인덱싱해 **자식·부모 두 저장소**를 만듭니다. 페이지 단위 원문은 부모로 `ex11/data/parent_docs.json`에 두고, 220자로 잘게 쪼갠 자식 청크는 ChromaDB에 `parent_id` 메타데이터와 함께 적재합니다. PDF는 `src/tuning/hybrid_parser.py`(챕터 10의 하이브리드 파서를 이식한 모듈)가 pypdf로 먼저 꺼내 보고 스캔본이면 Vision LLM으로 넘어가 텍스트를 뽑습니다. BM25 인덱스와 Cross-Encoder 모델은 각 객체가 최초 사용 시 한 번만 초기화합니다.
...
*그림 11-2. 같은 UI에서 질문의 성격에 따라 근거 모양이 자동으로 바뀝니다. 숫자는 숫자로, 문장은 이미지로*
...
<div class="sp-figure">
<div class="sp-figure-title">그림 11-12. 11챕터의 여정. 환각에서 출발해 시스템으로 끝났습니다</div>
