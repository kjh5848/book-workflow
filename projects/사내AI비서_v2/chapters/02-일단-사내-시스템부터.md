# Ch.2: "일단 사내 시스템부터" — 사내 시스템 소개 (v0.2)
...
그래서 이번 챕터에서는 AI 비서보다 먼저, AI 비서가 조회할 **사내 시스템**을 만든다. 코드를 하나하나 뜯어보진 않는다. 완성된 시스템을 실행해보고, "이런 데이터를 이렇게 조회할 수 있구나"를 확인하는 게 목표다.
...
> 사람이 Swagger UI에서 버튼을 누르듯이, AI 비서도 API를 호출한다. CH06에서 **MCP(Model Context Protocol)** 라는 도구를 통해 AI가 직접 API를 호출하는 법을 다룬다. 지금은 "AI가 쓸 시스템을 먼저 만들어두는 것"에 집중하자.
...
이 세 테이블의 데이터를 API로 관리하는 시스템. 그게 이번 챕터에서 확인할 것이다.
...
> **참고: 실습 환경이 처음이라면**
> 부록의 "환경 설정" 챕터를 먼저 참고하세요. Python 가상환경, Docker 설정을 다룹니다.
...
브라우저에서 `http://localhost:8000/docs`를 열면 **Swagger UI**가 뜬다. FastAPI가 코드에서 자동으로 만들어주는 API 문서다.
...
직원, 연차, 매출 — 세 영역의 API가 보인다. 직접 눌러보자.

POST로 직원을 등록하고, GET으로 조회하면 방금 등록한 데이터가 돌아온다. 수정(PATCH), 삭제(DELETE)도 된다. 이야기 파트에서 말한 CRUD 네 가지가 전부 동작하는 것이다.

<!--
[CAPTURE NEEDED]
path: assets/CH02/02_api-test-employee.png
Swagger UI에서 POST /api/employees로 직원을 등록하고 GET으로 조회하는 브라우저 화면
-->
![API 테스트](../assets/CH02/02_api-test-employee.png)
*그림 2-5: 직원 등록(POST) 후 조회(GET) 결과. CRUD가 정상 동작한다.*

Swagger UI는 개발자용이다. 하지만 이 시스템에는 일반 사용자를 위한 웹 UI도 있다. 브라우저에서 `http://localhost:8000/admin/`을 열어보자.
...
![관리자 대시보드](../assets/CH02/02_admin-dashboard.png)
*그림 2-6: Jinja2 템플릿으로 만든 관리자 대시보드. 직원, 연차, 매출 현황을 한눈에 볼 수 있다.*

직원 관리 메뉴에서 사번, 이름, 부서, 직급, 입사일을 입력하고 등록하면 — 아래 목록에 바로 나타난다.
...
![직원 등록 UI](../assets/CH02/02_admin-employee-create.png)
*그림 2-7: 웹 UI에서 직원을 등록하면 목록에 바로 반영된다. API를 몰라도 CRUD가 된다.*

API는 뒷단의 배관이고, 웹 UI는 수도꼭지다. 사용자는 수도꼭지만 틀면 되고, 물이 어떤 배관을 타고 오는지 몰라도 된다. 나중에 AI 비서도 같은 배관(API)을 사용한다 — 다만 수도꼭지 대신 코드로 틀 뿐이다.

---

### API 엔드포인트 목록

이 시스템이 제공하는 API 전체 목록이다. CH06에서 AI 비서가 MCP로 이 API들을 호출하게 된다.
...
> **팁: 코드가 궁금하다면**
> `code/v0.2/app/` 폴더에 전체 소스가 있다. FastAPI + psycopg2 + Pydantic 조합으로 만들어져 있다. 이 책의 주제가 아니므로 코드 설명은 생략하지만, 관심 있으면 직접 읽어봐도 좋다.

---

### 더 알아보기

**Swagger UI** — FastAPI는 코드에서 API 문서를 자동 생성한다. Pydantic 스키마에 적어둔 필드 설명과 타입이 그대로 문서에 나온다. `/docs`는 Swagger UI, `/redoc`은 ReDoc 스타일로 볼 수 있다.

**DeptSummary** — `GET /api/sales/dept-summary`는 부서별 매출 합계를 반환한다. CH06에서 AI 비서의 `sales_sum` 도구가 이 엔드포인트를 호출해서 "개발팀 매출 얼마야?"에 답한다.

---

### 이것만은 기억하자

- **AI 비서가 조회할 사내 시스템이 준비됐다.** API는 식당 웨이터처럼 요청을 받아 DB에서 데이터를 가져다준다.
