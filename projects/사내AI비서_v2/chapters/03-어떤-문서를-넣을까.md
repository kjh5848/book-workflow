...
> 한 줄 요약: AI에게 좋은 답을 원하면 좋은 문서를 넣어야 한다. 도서관처럼 분류하고 라벨을 붙이자.<br>
...
CH01에서 더미 문서 3개로 RAG를 돌려봤습니다. 잘 됐죠. 그런데 이제 실제 사내 문서를 넣어야 합니다. 공유 드라이브를 열었습니다.

취업규칙.pdf, 보안지침_v3_최종_진짜최종.docx, 2024년_복지정책.xlsx, 회의록_0301.hwp, 프로젝트_보고서.pptx...
...
재인덱싱을 안 하면 어떻게 될까요? AI 비서가 폐기된 규정을 근거로 답변합니다. "연차 15일입니다"라고 답하는데 실제로는 규정이 바뀌어서 20일인 상황이에요. 환각보다 더 위험합니다. 출처까지 달려있으니 그걸 신뢰하게 되거든요.
...
별도의 메타데이터 파일을 만들 필요가 없습니다. **폴더 구조와 파일명이 곧 메타데이터** 니까요. 다음 챕터에서 파서가 이 정보를 자동으로 추출해 벡터 DB에 저장합니다.
