...
다음 날 아침이었습니다. 옆자리 동료가 의자를 돌려 오픈이를 봤습니다.

**동료**: "어제 비서한테 연차 물어봤다고 했잖아요."<br>
**동료**: "15일이라고 답했다면서요. 인사팀 가니까 20일이래요. 혼났어요."
...
환각은 "모르는데 지어낸" 것이라 의심이라도 하지만, 이건 **출처까지 달려 있으니 사용자가 그대로 믿어 버립니다.** 동료는 폐기본을 근거로 인사팀에 가서 혼났고, 커넥트HR 에이전트가 그 과정에 공범이 된 셈이었습니다.
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 3-3. 넣을 문서와 뺄 문서</div>
  <div class="sp-compare">
    <div class="sp-compare-block good">
      <div class="sp-compare-label">넣어야 할 것</div>
      <div class="sp-compare-content">
        <ul>
          <li>현재 유효한 규정·정책·가이드</li>
          <li>자주 질문받는 내용이 담긴 문서</li>
          <li>최신 버전임이 명확한 파일</li>
        </ul>
      </div>
    </div>
    <div class="sp-compare-block bad">
      <div class="sp-compare-label">빼야 할 것</div>
      <div class="sp-compare-content">
        <ul>
          <li>폐기·만료된 문서</li>
          <li>개인 메모·작업 노트</li>
          <li>같은 내용의 v1·v2·v3 중복본</li>
        </ul>
      </div>
    </div>
  </div>
...
판단이 망설여지면 기준 한 줄이면 충분합니다. **"이 문서를 신입사원에게 줘도 되나?"** 그렇다면 넣고, 아니라면 뺍니다.

## 3.4 형식 통일. Markdown으로 번역하기

PDF, DOCX, XLSX, HWP는 그대로 벡터 DB에 넣을 수 없습니다. 벡터 DB는 **텍스트**만 이해하기 때문입니다.

해외 지사 보고서를 떠올려 보겠습니다. 미국 지사는 영어로, 일본 지사는 일본어로, 프랑스 지사는 프랑스어로 문서를 보냈습니다. 우리 팀 누구나 검색하고 읽으려면 먼저 **한국어로 번역**해 하나의 언어로 통일해야 합니다. PDF, DOCX, XLSX도 똑같습니다. 형식이 제각각이면 벡터 DB가 읽지 못합니다. 먼저 **하나의 텍스트 형태**로 바꿔야 합니다.

이 책에서는 **Markdown**으로 통일합니다. 이유는 네 가지입니다.

<div class="sp-figure">
  <div class="sp-figure-title">그림 3-4. Markdown으로 통일하는 네 가지 이유</div>
  <div class="sp-row accent">
    <span class="sp-row-label">LLM 친화</span>
    <span class="sp-row-value">훈련 데이터에 Markdown이 대량 포함돼 <code># 제목</code>이나 <code>- 목록</code> 구조를 자연스럽게 인식</span>
  </div>
  <div class="sp-row accent">
    <span class="sp-row-label">제목 보존</span>
    <span class="sp-row-value">PDF의 큰 글씨, DOCX의 "제목 1" 스타일이 <code># 제목</code>으로 변환</span>
  </div>
  <div class="sp-row accent">
    <span class="sp-row-label">표 보존</span>
    <span class="sp-row-value">엑셀 시트가 <code>| 열1 | 열2 |</code> 형태로 변환</span>
  </div>
  <div class="sp-row accent">
    <span class="sp-row-label">사람도 읽기 가능</span>
    <span class="sp-row-value">변환 결과가 제대로인지 눈으로 바로 검증</span>
  </div>
</div>

![](../assets/CH03/diagram/03_parser-flow.png)
*그림 3-5. 파서가 다양한 형식을 Markdown 텍스트로 통일합니다. 그래야 청킹하고 검색할 수 있습니다*
...
<div class="reindex-compare">
  ...
  <div class="rc-caption">그림 3-6. 별표(✱)는 개정된 문서. 전체 재인덱싱은 멀쩡한 네 개까지 지웠다가 다시 넣고, 증분 재인덱싱은 바뀐 하나만 갈아 끼웁니다</div>
</div>
...
<div class="arch-fullmap">
  <div class="arch-fullmap-title">전체 구성도. 짙은 박스가 챕터 3 범위</div>

  <div class="afm-row afm-user">
    <div class="afm-box afm-built afm-round">
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row">
      <div class="afm-box afm-built">
        <div class="afm-label">FastAPI</div>
        <div class="afm-sub">REST API · 관리자 웹</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row afm-three">
      <div class="afm-box afm-on afm-dashed">
        <div class="afm-tag">오늘 설계한 규칙</div>
        <div class="afm-label">챕터 3 문서 규칙</div>
        <div class="afm-sub">PDF·Word·Excel·HWP</div>
      </div>
      <div class="afm-box afm-faint"><div class="afm-label">Doc Pipeline</div><div class="afm-sub">파싱·청킹·임베딩</div></div>
      <div class="afm-box afm-faint"><div class="afm-label">벡터 DB</div><div class="afm-sub">벡터 저장소</div></div>
    </div>
  </div>
  ...
  <div class="afm-note">
    오늘은 코드 대신 <b>규칙</b>을 설계했습니다. 챕터 4에서 이 규칙을 따르는 문서를 실제로 파싱, 청킹해서 <b>벡터 DB</b>에 저장합니다.
  </div>
</div>
...
- **폴더 구조와 파일명이 곧 메타데이터입니다.** `분류_제목_버전` 규칙만 지키면 파서가 자동으로 메타데이터를 뽑아 냅니다. 다음 챕터에서 이 규칙을 따라 정리된 문서를 실제로 파싱·청킹·임베딩해서 벡터 DB에 올립니다.
:::
