...
입사 3일 차였다.<br>
아직 사내 Wi-Fi 비밀번호를 포스트잇에 적어 모니터에 붙여놓던 시절이었다.

오전 10시, 팀장이 자리로 왔다.

"AI로 사내 문서 검색 시스템 만들어봐."<br>
"직원들이 규정이나 정책 찾는 게 너무 번거롭다고 해서."<br>
"채팅창에 물어보면 바로 답해주는 거."
...
그런데 입사할 때 받은 규정집과 비교해보니 완전히 달랐다.
커넥트의 실제 규정은 이랬다.
...
LLM을 이렇게 생각해 보겠습니다. 입사 면접을 보러 온 외부인입니다. 이 외부인은 세상에 공개된 거의 모든 자료를 읽었습니다. 인터넷, 뉴스, 책, 논문 — 공개된 텍스트라면 뭐든지. 그래서 근로기준법은 완벽하게 알고, 일반적인 회사 연차 제도도 줄줄 외웁니다.

그런데 커넥트의 내부 규정집은 공개된 적이 없다. 이 외부인이 읽을 방법이 없었다.

문제는 이 외부인이 "모른다"고 솔직히 말하지 못한다는 겁니다. 질문을 받으면 자기가 아는 것들 중에서 가장 비슷해 보이는 걸 자신감 있게 말합니다. "아마 일반적인 회사라면 이렇겠지"라는 추측이지만, 마치 확실히 아는 것처럼 들립니다.

이게 **LLM 환각(Hallucination)** 입니다.

GPT든 Claude든 Gemini든 마찬가지입니다. 자기가 학습한 데이터에 없는 정보는 알 방법이 없습니다. 그런데 그걸 솔직히 "모른다"고 하지 않고, 그럴듯하게 지어냅니다. 특히 일반적인 내용과 비슷한 맥락일수록 더 자연스럽게 지어냅니다.

커넥트의 연차 규정은 공개된 인터넷 어디에도 없습니다. 그러니 LLM이 알 방법이 없습니다. 근로기준법 기반으로 그럴듯한 답을 만들어낸 것뿐입니다.
...
LLM에는 한 번에 처리할 수 있는 텍스트 길이 한도가 있습니다. 문서가 쌓일수록 한도를 넘기기 쉽습니다. 그리고 무엇보다, 연차 규정을 물어보는데 보안 지침이나 복지 정책까지 다 넣어서 보내는 건 비효율적입니다. 관련 없는 내용이 많을수록 LLM이 정작 필요한 내용을 찾기 어려워집니다.
...
이제 LLM이 우리 회사 규정을 외울 필요가 없습니다. 질문할 때마다 관련된 규정을 찾아서 보여주면 되니까요. 그리고 어느 문서를 참고했는지도 함께 돌려줄 수 있습니다.

이번 챕터의 목표는 이 흐름을 직접 만들어보는 것입니다. 지금은 더미 문서 3개짜리 간단한 버전으로 시작합니다. 실제 PDF 파싱, 한국어 임베딩 모델 적용, DB 연동은 뒤 챕터에서 차례로 붙입니다.
...
**ChromaDB** — 로컬에서 Python 패키지만으로 바로 쓸 수 있는 벡터 DB입니다. 이 챕터에서는 메모리 내에서만 동작하므로 프로그램을 재시작하면 저장된 내용이 사라집니다. CH04에서 디스크에 영구 저장하는 방식으로 바꿉니다.
