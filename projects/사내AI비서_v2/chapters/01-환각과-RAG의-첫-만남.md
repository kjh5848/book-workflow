...
-->
![컨텍스트 넘침](../assets/CH01/gemini/01_context-overflow.png)
...
답변을 읽어보면 "근로기준법에 따라 1년 미만은 매월 1일..." 같은 내용이 나옵니다. 공식적인 느낌도 나고 그럴듯하죠. 하지만 이야기 파트에서 봤듯이 커넥트의 실제 규정은 완전히 다릅니다. LLM은 커넥트라는 회사를 모릅니다. 학습 데이터에 없으니 일반적인 근로기준법 내용을 가져다 붙인 겁니다. 이게 **환각(Hallucination)** 입니다. 틀린 답을 자신감 있게 내놓았고, 출처도 없습니다.
...
이번에는 "3년 동안 연차 없음, 매월 리프레시 데이 1회"라는 커넥트의 실제 규정이 답변에 나옵니다. step1과 코드 구조는 거의 같은데 결과가 완전히 달라졌습니다. 달라진 건 `context_data`를 프롬프트에 넣었다는 것뿐이에요. LLM에게 정답지를 건네준 셈이죠. 환각이 사라진 건 좋지만, 이 방식은 문서가 늘어날수록 한계가 뚜렷합니다. 수십 개 문서를 매번 전부 붙일 수는 없으니까요.
...
흐름은 네 단계입니다.
1. **벡터 DB 저장** — `OllamaEmbeddings`가 각 문서를 수백 차원의 숫자 배열(벡터)로 변환합니다. `Chroma.from_documents()`가 이 벡터를 ChromaDB에 저장하고요.
2. **검색기 설정** — `k=3`은 "질문과 가장 비슷한 문서 3개를 가져오라"는 설정입니다.
3. **RAG 체인 연결** — `RetrievalQA`가 검색기와 LLM을 체인으로 연결합니다.
...
![청킹 비교 결과](../assets/CH01/terminal/01_no-chunking-compare.png)
