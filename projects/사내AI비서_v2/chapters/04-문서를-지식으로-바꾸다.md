# Ch.4: "문서를 '지식'으로 바꾸다" — VectorDB 구축 (v0.3)
...
VectorDB 구축 파이프라인을 요리 과정에 비유한 인포그래픽.
왼쪽부터 오른쪽으로:
1단계: 마트 봉지(PDF/DOCX/XLSX 아이콘) → 손질(칼로 자르는 손, "파싱")
2단계: 도마 위 재료 → 다지기(잘게 썬 조각들, "청킹")
3단계: 양념통들 → 밑간(숫자 벡터로 변환, "임베딩")
4단계: 깔끔한 냉장고 → 라벨 붙은 용기들("벡터 DB")
각 단계 사이에 화살표. 전체적으로 밝고 친근한 일러스트 스타일.
Style: step-by-step infographic, 한글 레이블, warm kitchen colors
-->
...
`python src/main.py` 실행 결과 터미널 스크린샷.
6개 문서 파싱 → 34개 청크 생성 → ChromaDB 저장 완료까지 전체 출력.
-->

6개 문서가 34개 청크로 변환되어 벡터 DB에 들어갔다. 이제 검색해보자.

---

### 검색해보자 — "연차 사용 규정"

CLI 검색 도구로 "연차 사용 규정"을 검색하면:
...
`python src/cli_search.py --query "연차 사용 규정" --top-k 3` 실행 결과.
유사도 점수 + 프로그레스 바 + 출처 표시 포함.
-->
...
└── src/
    ├── extractor.py    [실습] 형식별 텍스트 추출 통합 모듈
    ├── chunker.py      [실습] Fixed-size 청킹 + 메타데이터 부착
    ├── store.py        [실습] ko-sroberta 임베딩 + ChromaDB 저장/검색
    ├── main.py         [설명] 파이프라인 오케스트레이터
    ├── cli_search.py   [설명] 벡터 검색 CLI 검증 도구
...
### [실습] chunker.py — Fixed-size 청킹 + 메타데이터
...
### [실습] store.py — 임베딩 + ChromaDB 저장/검색
