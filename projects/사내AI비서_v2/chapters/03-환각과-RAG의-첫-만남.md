# Ch.3: "ChatGPT에 물어봤더니…" — 환각과 RAG의 첫 만남 (v0.0)
...
GPT든 Claude든 Llama든 마찬가지다. 자기가 학습한 데이터에 없는 정보는 알 방법이 없다. 그런데 그걸 솔직히 "모른다"고 하지 않고, 그럴듯하게 지어낸다. 특히 일반적인 내용과 비슷한 맥락일수록 더 자연스럽게 지어낸다.
...
프롬프트에 규정 내용을 직접 붙여서 다시 물어봤다. *"아래 [커넥트 취업규칙]을 참고해서 신입사원 연차 규정을 알려줘"* 라고 앞에 붙이고, 규정 내용 전체를 그 뒤에 이어 붙였다.
...
<!-- [GEMINI PROMPT: ch03_rag_flow]
path: assets/CH03/ch03_rag_flow.png
LLM 단독 vs RAG 비교 다이어그램. 왼쪽: 질문 → LLM → 환각 답변. 오른쪽: 질문 → 검색기 → 벡터DB → 관련 문서 조각 → LLM → 출처 포함 답변. 깔끔한 흐름도 스타일.
Style: architecture-infographic
-->
![LLM 단독 vs RAG 비교](../assets/CH03/ch03_rag_flow.png)
*그림 3-1: LLM 단독 호출과 RAG의 차이. RAG는 질문마다 관련 문서를 찾아서 LLM에 건네준다.*
...
> 실제 PDF/DOCX 파싱과 영속 저장은 CH06(VectorDB 구축)에서 다룬다.
...
<!-- [CAPTURE NEEDED: ch03_step1_hallucination
  path: assets/CH03/ch03_step1_hallucination.png
  desc: step1_fail.py 실행 결과 — LLM이 존재하지 않는 커넥트 규정을 그럴듯하게 답하는 화면
] -->
![step1 실행 결과](../assets/CH03/ch03_step1_hallucination.png)
*그림 3-2: step1_fail.py 실행 결과. 자신감 있게 답하지만 실제 커넥트 규정과 다르다.*
...
<!-- [CAPTURE NEEDED: ch03_step2_context
  path: assets/CH03/ch03_step2_context.png
  desc: step2_context.py 실행 결과 — 규정 내용을 직접 주입했을 때 정확한 답변이 나오는 화면
] -->
![step2 실행 결과](../assets/CH03/ch03_step2_context.png)
*그림 3-3: step2_context.py 실행 결과. 문서를 직접 넣으니 정확하게 답한다.*
...
> **이 챕터의 임베딩 모델**: `nomic-embed-text`를 사용한다. CH06에서 한국어에 최적화된 `ko-sroberta-multitask`로 교체할 것이다.
...
<!-- [CAPTURE NEEDED: ch03_step3_rag
  path: assets/CH03/ch03_step3_rag.png
  desc: step3_rag.py 실행 결과 — 검색된 문서 출처와 함께 정확한 답변이 나오는 화면
] -->
![step3 실행 결과](../assets/CH03/ch03_step3_rag.png)
*그림 3-4: step3_rag.py 실행 결과. [인사규정] 문서를 찾아서 답변하고, 어디서 가져왔는지 출처까지 보여준다.*
...
`step3_rag_no_chunking.py`는 모든 문서를 하나의 덩어리로 합쳐서 저장한다. `step3_rag.py`와 비교하면 무슨 차이가 있을까? 직접 실행해서 검색 결과를 비교해보자. 왜 문서를 조각으로 나눠야 하는지 체감할 수 있다. 청킹 전략의 상세 비교는 CH10(검색 품질 튜닝)에서 다룬다.
...
**RetrievalQA vs LCEL** — 이 챕터에서는 LangChain의 `RetrievalQA` 체인을 사용했다. 구버전 API지만 구조가 직관적이라 처음 RAG를 이해하기 좋다. CH07에서 더 유연한 LCEL(LangChain Expression Language) 파이프라인으로 교체한다.

**ChromaDB** — 로컬에서 Python 패키지만으로 바로 쓸 수 있는 벡터 DB다. 이 챕터에서는 메모리 내에서만 동작하므로 프로그램을 재시작하면 저장된 내용이 사라진다. CH06에서 디스크에 영구 저장하는 방식으로 바꾼다.
