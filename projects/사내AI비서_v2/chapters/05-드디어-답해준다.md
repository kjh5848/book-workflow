...
- [x] v0.0: LLM 환각 체험 + RAG 맛보기
- [x] v0.1: FastAPI로 사내 시스템(직원/연차/매출) CRUD
- [x] v0.2: 사내 문서 수집 전략과 표준 설계
- [x] v0.3: 문서 파싱 → 청킹 → 임베딩 → 벡터 DB 구축
- [ ] **v0.4: RAG Q&A 엔진** ← 이번 챕터
...
v0.3(벡터 검색 CLI)에서 v0.4(RAG Q&A 엔진)로 — 검색만 하던 시스템이 드디어 **답변**을 해준다.

<!--
[CAPTURE NEEDED]
path: assets/CH05/05_rag-qa-result.png
v0.4 실행 결과. 질문 "병가 쓸 때 증빙 서류가 필요한가요?"에 대한 답변 + [출처: HR_취업규칙_v1.0] 표시.
-->
...
이번 챕터부터 LangChain이 등장한다. v0.3에서는 ChromaDB와 sentence-transformers만 직접 사용했지만, v0.4에서는 LangChain이 Retriever, Prompt, LLM을 파이프로 연결해준다.
...
FastAPI 서버 시작 로그. `[INFO] 서버 시작: http://0.0.0.0:8000` + ChromaDB 로드 메시지.
-->
...
curl 명령어 실행 결과. JSON 응답에 answer + sources 포함.
-->
