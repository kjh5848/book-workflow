...
<div class="sp-figure">
  <div class="sp-figure-title">그림 8-4. Recursive Separators 탐색 순서. 위에서 못 찾으면 아래로 내려갑니다</div>
  <div class="sp-flow">
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 8-5. percentile 기준선 비교. 같은 텍스트도 기준선에 따라 청크 수가 달라집니다</div>
...
*그림 8-6. 같은 1098자 문서에 세 전략을 적용한 결과*
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 8-7. 리랭킹 2단계 전략. Bi-Encoder(서류) → Cross-Encoder(면접)</div>
  <div class="sp-flow">
    <div class="sp-step info">
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 8-10. 정규화. 만점이 다른 두 점수를 0~1로 맞춰야 합칠 수 있습니다</div>
  <div class="sp-compare">
    <div class="sp-compare-block">
...
<div class="sp-figure">
  <div class="sp-figure-title">그림 8-11. alpha 가중치. 의미와 키워드 중 어느 쪽을 더 믿을지</div>
  <div class="sp-flow">
    <div class="sp-step warm">
...
`search()` 는 BM25와 Vector 각각에서 `fetch_k` 개를 가져온 뒤 합쳐서 `top_k` 개를 반환합니다. `_normalize_scores()` 는 점수를 0~1 범위로 맞추는 보조 함수입니다. 만점이 다른 두 검색의 점수를 합치려면 반드시 정규화가 먼저 필요합니다(그림 8-10 참조). `alpha` 는 두 검색의 비중을 조절하는 손잡이입니다. `hybrid_score = alpha × vector_score + (1 - alpha) × bm25_score` 공식으로 합산합니다.
