...
별생각 없이 기존 파이프라인에 넣습니다. pypdf로 텍스트 추출하고, 청킹하고, 벡터DB에 넣는 그 흐름. 그런데 추출 결과가 빈 문자열입니다.
...
> 실습 1~3은 `tuning/`에서 실험하고, 실습 4는 `src/`에 코드를 작성합니다. 기존 파일(`agent_config.py`, `tools/` 등)은 CH05~09에서 만든 코드로 수정하지 않습니다.
...
실험 3개를 순서대로 진행하고, 마지막으로 CH08~09 기술이 통합된 웹 UI를 직접 만듭니다.
...
지금까지 만든 기술을 하나의 웹 UI로 통합합니다. 이 실습에서는 `src/` 디렉토리의 **파일 2개에 코드를 작성**합니다.
...
PDF 페이지를 이미지로 저장하고 텍스트를 함께 추출하는 모듈입니다. import와 경로 상수(`BASE_DIR`, `DOCS_DIR`, `CAPTURED_DIR`)는 이미 파일에 준비되어 있습니다. `fitz`는 PyMuPDF 라이브러리입니다. `ex10/src/capture.py`의 TODO 부분을 다음과 같이 채웁니다.
...
캡처된 이미지의 절대 경로를 웹 브라우저에서 접근할 수 있는 URL로 변환하는 모듈입니다. import와 경로 상수(`BASE_DIR`, `CAPTURED_DIR`)는 이미 파일에 준비되어 있습니다. `ex10/src/evidence.py`의 TODO 부분을 다음과 같이 채웁니다.
