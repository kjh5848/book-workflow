...
def _build_vectorstore():
    ...
    이전 버전과 동일한 파이프라인: docs → 파싱 → 청킹 → 임베딩 → ChromaDB 저장.
...
def search_documents(query: str) -> list[dict]:
    ...
    문서를 파싱하여 자동 구축합니다 (이전 버전과 동일 파이프라인).
