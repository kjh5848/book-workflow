...
8. **문체 구분**: 독자에게 말하는 설명문은 **존댓말(-입니다/-합니다/-세요)**로 쓴다. 이야기 파트의 내러티브(과거형 서술), 캐릭터 대화, 내면 독백(괄호 안)은 그대로 유지한다.
   - 독자 직접 지시: `참고하자` → `참고하세요`, `생각해보자` → `생각해 보겠습니다`
   - 설명 문장: `~이다`, `~한다` → `~입니다`, `~합니다`
   - 섹션 제목: `이것만은 기억하자` → `이것만은 기억하세요`
   - **유지**: `"입사 3일 차였다."` 같은 과거형 내러티브, `(오, 이거면 되는 거 아니야?)` 같은 독백
9. **챕터 닫기**: 챕터의 **맨 마지막**(기술 파트 뒤)에 "이것만은 기억하세요" + 다음 버전 예고.
10. **기술은 저자가 결정**: AI가 제안, 저자가 최종 결정.
11. **실패 먼저**: 정답부터 알려주지 않는다. 실수 → 에러 → 해결 순서.
12. **이미지는 플레이스홀더**: 집필 시 `[GEMINI PROMPT]`(개념도) / `[CAPTURE NEEDED]`(실습 캡처) 삽입. 실제 이미지는 집필 완료 후 생성.
13. **스크린샷과 코드 블록 중복 금지**: 스크린샷이 확보되면 동일 내용의 출력 코드 블록 제거.
